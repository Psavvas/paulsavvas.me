...
if (contactForm) {
  contactForm.addEventListener('submit', function (e) {
    ...
    const name =
      formData.get('name') || this.querySelector('#name-field').value.trim();
    const email =
      formData.get('email') ||
      this.querySelector('input[type="email"]').value.trim();
    const subject =
      formData.get('subject') ||
      this.querySelector('#subject-field').value.trim();
    const message =
      formData.get('message') || this.querySelector('textarea').value.trim();
...
function typeWriter(element, text, speed = 100) {
  ...
  function type() {
    if (i < text.length) {
      ...
      element.innerHTML =
        '👋 Hi, I\'m <span class="highlight">Paul Savvas</span>';
    }
  }
  type();
}
